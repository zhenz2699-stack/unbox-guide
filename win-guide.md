# 搬瓦工 VPS 新手服务器入门（Win 版）
---

> **我推荐自己搭建是因为，我自己的设计工作就需要经常访问国外网站（pinterest 这个网站同行应该都懂），之前用 6、7 家都跑路了，从 21 年开始就自己搭建用了 5 年，它除了比那些跑路的稍微贵些（但算上当时发现他跑路的闹心就平复了？），就没其他毛病了。**

## 一、准备工作

1. 一台搬瓦工 VPS（ → [**搬瓦工官方购买链接**](https://bwh88.net/aff.php?aff=71166) ）

> -  日常个人使用推荐[洛杉矶节点](https://bwh88.net/aff.php?aff=71166&a=add&pid=44&billingcycle=annually&configoption[10]=35)，按年购买，一个月只要 2 杯奶茶钱（而且新注册如果不满意享受 30天 全额退款保障）

![这张图片是搬瓦工（BandwagonHOST）VPS主机的选购页面，页面顶部清晰](images/img-01.png)

1. 注册搬瓦工新用户账户，需要填写购买信息，如下图所示（除了邮箱和登录密码，**其他如实填写或者随意填写都可，不影响**），选择支付方式为**支付宝**，点击**Complete Order**即可跳转到支付页面：

![该图片展示的是搬瓦工注册流程中的账户信息填写页面，核心内容为用户详细信息填写区域](images/img-02.png)
...
![图片展示了搬瓦工KiwiVM控制面板的“客户区”页面](images/img-03.png)
![图片展示的是搬瓦工KiwiVM控制面板中“管理服务”页面](images/img-04.png)
![图片展示的是搬瓦工KiwiVM控制面板中服务器管理界面](images/img-05.png)
1. 建议下载 QoderWork辅助我们搭建（如果中间有哪些按流程执行不下去，直接截图给它基本就能帮忙安装完成）
...
![图片展示的是搬瓦工云服务器管理界面](images/img-06.png)
![这张图片是搬瓦工KiwiVM控制面板里的安装新操作系统页面，对应指南里选择22.](images/img-07.png)
---

## 三、连接服务器

在 Windows 电脑上打开 **PowerShell**（按 Win 键，搜索 PowerShell，回车），输入：
...
看到 `root@rave-box-1:~#` 这样的提示符就说明连接成功了。
...
> **注意：** 必须在 `root@xxx` 开头的服务器终端里运行（这代表你登录到了服务器），不要在本地 Windows PowerShell 里运行。
...
菜单出现后，输入 **3** 回车（一键无域名Reality）

### 第 2 步：选择核心

输入 **1** 回车（Xray-core）

> 不要选 sing-box，Windows 客户端 v2rayN 用的是 Xray 核心，保持一致避免兼容问题。
...
如果看到红色的 **"Xray启动失败"**，执行以下命令修复权限后再重启：
...
1. 输入 **5** 回车（REALITY管理）
2. 提示 `是否使用上次安装的配置？[y/n]` → 输入 **y** 回车
3. 提示 `是否使用上次安装时的端口？[y/n]` → 输入 **y** 回车
4. 提示 `是否使用Reality域名？[y/n]` → 输入 **y** 回车
5. 提示 `是否使用上次安装时的PublicKey/PrivateKey？[y/n]` → 输入 **y** 回车
6. 提示 `是否安装[y/n]?`（nginx 订阅）→ 输入 **n** 回车
7. 提示 `是否使用http订阅[y/n]?` → 输入 **n** 回车

脚本会显示一个 vless 开头的链接，类似：
...
1. 打开浏览器访问：https://github.com/2dust/v2rayN/releases
2. 找到最新版本，下载 **`v2rayN-windows-64.zip`**（体积最大的那个，约 149 MB，自带 Xray 核心）
3. 解压到一个目录（如 `D:\v2rayN`）
4. 双击运行 `v2rayN.exe`

> **注意：** 不要下载 `desktop` 版本，那个不带核心。选体积最大的 `v2rayN-windows-64.zip`。

> 💡
> 如果下载不了可以用网盘备份：win-通用版，链接 
> 百度网盘链接: https://pan.baidu.com/s/1KUWGtaq7tKNiqBHjNyKOOA 提取码: 5ada 
> 夸克网盘链接: https://pan.quark.cn/s/7d14f8105375 提取码：PayW

### 第 2 步：导入节点

1. 在 v2rayN 窗口中，点击菜单栏 **服务器**
2. 选择 **从剪贴板导入批量 URL**
3. 粘贴刚才保存的 vless 链接
4. 点击确定
...
1. **右键** 刚导入的节点
2. 选择 **测试延迟**（或按 `Ctrl+T`）
3. 如果延迟显示**绿色数字**（如 163ms），说明连接成功
4. 如果显示 **-1**（红色），说明连接失败，跳到第九章排查

### 第 4 步：启用代理

1. **双击** 该节点，设为活动服务器（行会变蓝）
2. 底部 **系统代理** 下拉菜单选择 **"自动配置系统代理"**
3. 底部 **路由** 下拉菜单选择 **"绕过大陆(Whitelist)"**
...
- 检查防火墙端口是否开放：在服务器执行 `ufw status`，确认端口在列表里
- 检查链接参数是否正确，特别是 `sid=` 不能为空
- 检查服务器 IP 和端口是否填对

### 问题 2：全局模式能上 Google，但绕过大陆模式不行

1. 在 v2rayN 点击菜单栏 **设置**
2. 选择 **检查更新**
3. 点击 **Geo files** 更新 geosite.dat 和 geoip.dat
4. 点击 **重启服务** 按钮
5. 切回 **绕过大陆** 模式再试
...
| 更新 GEO 数据库 | v2rayN 里：设置 → 检查更新 → Geo files |
